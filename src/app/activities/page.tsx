...
import { activities, activityTypes, type ActivityType } from '@/lib/activities'
  ...
  return (
    <div className="mx-auto min-h-screen w-full max-w-6xl px-4 py-8">
      <header>
        ...
        <p className="mt-1 text-sm text-white/70">Filter by type and date, then bookmark what you like.</p>
      </header>
      ...
      <section className="mt-6 grid gap-4 sm:grid-cols-2">
        {filteredActivities.map((activity) => {
          ...
          return (
            <article key={activity.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <p className="mt-1 text-xs text-white/60">{activity.location}</p>
      ...
      <div className="mt-6 rounded-xl border border-white/10 bg-white/5 p-4 text-sm text-white/70">
        Need help choosing? <Link href="/ai" className="text-yellow-400">Use the AI guide</Link>.
      </div>
    </div>
  )
}
